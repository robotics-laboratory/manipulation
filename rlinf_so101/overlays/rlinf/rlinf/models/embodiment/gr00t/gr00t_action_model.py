...
class GR00T_N1_5_ForRLActionPrediction(GR00T_N1_5, BasePolicy):
    ...
    @staticmethod
    def _stats(min_values: list[float], max_values: list[float]) -> dict[str, list[float]]:
        mean = [(low + high) / 2.0 for low, high in zip(min_values, max_values)]
        std = [max((high - low) / 4.0, 1e-6) for low, high in zip(min_values, max_values)]
        return {
            "max": max_values,
            "min": min_values,
            "mean": mean,
    ...
    @classmethod
    def _default_so101_metadata(cls) -> dict[str, Any]:
        arm_min = [-100.0, -100.0, -100.0, -100.0, -100.0]
        arm_max = [100.0, 100.0, 100.0, 100.0, 100.0]
        gripper_min = [0.0]
        gripper_max = [100.0]
        return {
            "statistics": {
                "state": {
                    "single_arm": cls._stats(arm_min, arm_max),
                    "gripper": cls._stats(gripper_min, gripper_max),
                },
                "action": {
                    "single_arm": cls._stats(arm_min, arm_max),
                    "gripper": cls._stats(gripper_min, gripper_max),
    ...
    def _load_metadata(self, exp_cfg_dir: Path):
        ...
        if metadata_dict is None:
            if self.embodiment_tag.value == "so101":
                metadata_dict = self._default_so101_metadata()
            else:
                raise ValueError(
