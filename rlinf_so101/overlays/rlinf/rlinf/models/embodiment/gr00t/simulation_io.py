...
def convert_so101_lift_cube_obs_to_gr00t_format(env_obs):
    """Convert SO-101 LiftCube observations to GR00T's LeRobot-style format."""
    states = env_obs["states"].unsqueeze(1).cpu().float().numpy()
    return {
        "video.front": env_obs["main_images"].unsqueeze(1).cpu().numpy(),
        "video.wrist": env_obs["wrist_images"].unsqueeze(1).cpu().numpy(),
        "state.single_arm": states[:, :, :5],
        "state.gripper": states[:, :, 5:6],
...
def convert_to_so101_lift_cube_action(
    action_chunk: dict[str, np.array], chunk_size: int = 16
) -> np.ndarray:
    """Convert GR00T SO-101 LeRobot motor-unit actions to LeIsaac joint radians."""
    action_array = np.concatenate(
        [
            action_chunk["action.single_arm"][:, :chunk_size],
            action_chunk["action.gripper"][:, :chunk_size],
        ],
        axis=-1,
    )
    assert action_array.shape[-1] == 6, (
        f"Expected 6-dim SO-101 action, got {action_array.shape[-1]}"
    )
    joint_lows = np.array([-110.0, -100.0, -100.0, -95.0, -160.0, -10.0])
    joint_highs = np.array([110.0, 100.0, 90.0, 95.0, 160.0, 100.0])
    motor_lows = np.array([-100.0, -100.0, -100.0, -100.0, -100.0, 0.0])
    motor_highs = np.array([100.0, 100.0, 100.0, 100.0, 100.0, 100.0])
    motor_fraction = (action_array - motor_lows) / (motor_highs - motor_lows)
    joint_degrees = motor_fraction * (joint_highs - joint_lows) + joint_lows
...
OBS_CONVERSION = {
    ...
    "so101_lift_cube": convert_so101_lift_cube_obs_to_gr00t_format,
}
...
ACTION_CONVERSION = {
    ...
    "so101_lift_cube": convert_to_so101_lift_cube_action,
}
