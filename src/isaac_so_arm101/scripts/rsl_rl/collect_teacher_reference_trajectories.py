...
parser.add_argument(
    ...
    type=int,
    default=12,
    help="Number of extra rollout steps to keep after first success-threshold crossing.",
)
...
parser.add_argument(
    ...
    default=True,
    help=(
        "If enabled, inspect actor input dim in checkpoint and auto-enable legacy dense "
        "observations when the actor expects 28-D input (pose command term)."
    ),
)
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Force legacy dense observations (commands + generated object_pose command in policy obs).",
)
parser.add_argument(
    "--success_only",
    action="store_true",
...
def _enable_legacy_dense_obs28(env_cfg: ManagerBasedRLEnvCfg) -> None:
    """Match older dense SO-101 teachers that used UniformPoseCommand in the observation vector."""
    from leisaac.tasks.lift_cube.lift_cube_env_cfg import LiftCubeCommandsCfg

    env_cfg.commands = LiftCubeCommandsCfg()
    env_cfg.observations.policy.cube_position = ObsTerm(
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if args_cli.auto_match_checkpoint_obs and not use_legacy_obs28:
        ...
        if checkpoint_obs_dim == 28:
            ...
            print("[collector] Detected teacher actor input dim=28. Enabling legacy dense observations (command term).")

    if use_legacy_obs28:
        if not isinstance(env_cfg, ManagerBasedRLEnvCfg):
            raise RuntimeError("Legacy observation patch requires ManagerBasedRLEnvCfg.")
        _enable_legacy_dense_obs28(env_cfg)
    # This script saves NPZ trajectories only; disable recorder exporters even for *-Collect tasks.
    ...
    try:
        ...
        if "size mismatch for actor.0.weight" in str(exc):
            raise RuntimeError(
                ...
                f"Heuristic checkpoint actor input dim: {checkpoint_obs_dim}. "
                "Try --force-legacy-dense-obs28 for older dense SO-101 checkpoints that used pose commands."
            ) from exc
        raise
    ...
    while simulation_app.is_running() and collected < args_cli.num_episodes:
        ...
        if not success_detected and h_ab > args_cli.height_success_threshold_m:
            success_detected = True
            post_success_step_count = 0
            print(
                f"[collector] Success threshold crossed at step={step_count} "
                f"(h_ab={h_ab:.4f}m > {args_cli.height_success_threshold_m:.4f}m); "
                f"recording {args_cli.post_success_steps} post-success steps."
            )
        if success_detected:
            post_success_step_count += 1

        done = bool(dones[0].detach().cpu().item()) if torch.is_tensor(dones[0]) else bool(dones[0])
        force_timeout = step_count >= args_cli.max_steps

        episode_finished = False
        success = False
        timeout = False
        if success_detected and post_success_step_count >= args_cli.post_success_steps:
            success = True
            episode_finished = True
            # For *-Collect configs success termination is disabled; close the episode explicitly.
            if not done:
                obs, _extras = env.reset()
        elif done or force_timeout:
            episode_finished = True
            if force_timeout and not done:
                ...
                # done=True can be auto-success auto-reset in non-collect tasks
