...
def _to_quat_wxyz(orient, field_name: str) -> tuple[float, float, float, float]:
    if not isinstance(orient, (list, tuple)):
        raise ValueError(f"'{field_name}' must be a list/tuple")
    if len(orient) == 3:
        return _euler_xyz_deg_to_quat_wxyz(_to_xyz(orient, field_name))
    if len(orient) == 4:
...
def _apply_single_camera(scene, camera_key: str, camera_cfg_data: dict):
    ...
    quat_wxyz = _to_quat_wxyz(orient, f"{camera_key}.orient")
